...
void Wavefunc_in_pw::make_table_q(std::vector<std::string>& fn, ModuleBase::realArray& table_local)
{
    ModuleBase::TITLE("Wavefunc_in_pw", "make_table_q");

    if (fn.size() != static_cast<size_t>(GlobalC::ucell.ntype))
    {
        ModuleBase::WARNING_QUIT("Wavefunc_in_pw::make_table_q",
                                 "maybe NUMERICAL_ORBITAL is not read in, please check.");
    }
        ...
        for (int L = 0; L < GlobalC::ucell.atoms[it].nwl + 1; L++)
        {
            for (int N = 0; N < GlobalC::ucell.atoms[it].l_nchi[L]; N++)
            {
                ...
                while (!find)
                {
                    if (in.eof())
                    {
                        GlobalV::ofs_warning << "\n Can't find l=" << L << " n=" << N << " orbital." << std::endl;
                        ModuleBase::WARNING_QUIT("Control_Overlap", "Read_");
                    }
...
double Wavefunc_in_pw::smearing(const double& energy_x, const double& ecut, const double& beta)
{
    double w = 0.0;
...
void Wavefunc_in_pw::integral(const int meshr,
                              const double* psir,
                              const double* r,
                              const double* rab,
                              const int& l,
    ...
    for (int it = 0; it < GlobalC::ucell.ntype; it++)
    {
        for (int ia = 0; ia < GlobalC::ucell.atoms[it].na; ia++)
        {
            ...
            for (int L = 0; L < GlobalC::ucell.atoms[it].nwl + 1; L++)
            {
                ...
                for (int N = 0; N < GlobalC::ucell.atoms[it].l_nchi[L]; N++)
                {
                    //					GlobalV::ofs_running << " it=" << it << " ia=" << ia << " L=" << L << " N=" << N
                    //<< std::endl;
                                ...
                                { // atomic_wfc_so_mag
                                    ...
                                    for (int m = 0; m < 2 * L + 1; m++)
                                    {
                                        ...
                                        for (int ig = 0; ig < npw; ig++)
                                        {
                                            ...
                                            fdown
                                                = ModuleBase::IMAG_UNIT * sin(0.5 * (alpha + ModuleBase::PI)) * aux[ig];
                                            psi(iwall + 2 * L + 1, ig)
                                                = (cos(0.5 * gamma) + ModuleBase::IMAG_UNIT * sin(0.5 * gamma)) * fup;
